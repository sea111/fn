    ...
    $.fn.topdapengctrl = function (options) {
        ...
        var methods = {
            ...
            sensorDataCurve: function () {
                $("#jdLine").html("<h3 style='line-height: 338px;'>正在加载中...</h3>");
                $.ajax({
                    type: "get",
                    url: "js/jdhj.json",
                    dataType: "json",
                    ...
                    success: function (data) {
                        if (data.code != 200) {
                            $('#jdLine').html('<center><h3 style="line-height: 338px;">' + data.msg + '</h3></center>');
                        ...
                        $.fn.topcharts({
                            data: data,
                            renderTo: 'jdLine',//折线容器
                            callback: function (chart) {
                                $('.sensors').children("li").children("lable").find('input[name="sensor_latest"]').each(function (i) {
                                    $(this).off('change').on('change', function () {
                                        var $checked = $(this).prop('checked');
                                        //var dataType = $('#dataShow .current').attr('id');
                                          //if (dataType == 'btn_data_line') {//曲线
                                            var seriesId = 'series_' + $(this).val();
